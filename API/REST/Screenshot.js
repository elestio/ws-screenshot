...
exports.handler = async (event, context, callback) => {
  ...
  if (
    hardcodedAPIKey != "" &&
    event.queryStringParameters.apiKey != hardcodedAPIKey
  ) {
    callback(null, {
      status: 400,
  ...
  if (event.queryStringParameters.isFullPage == "true") {
    isFullPage = true;
  }
  var resX = 1280;
  if (event.queryStringParameters.resX != null) {
    resX = event.queryStringParameters.resX;
  }
  var resY = 900;
  if (event.queryStringParameters.resY != null) {
    resY = event.queryStringParameters.resY;
  }
  var outFormat = "jpg";
  if (event.queryStringParameters.outFormat != null) {
    outFormat = event.queryStringParameters.outFormat;
  }
  var orientation = "portrait";
  if (event.queryStringParameters.orientation != null) {
    orientation = event.queryStringParameters.orientation;
  }
  var waitTime = 100;
  if (event.queryStringParameters.waitTime != null) {
    waitTime = event.queryStringParameters.waitTime;
  }
  var headers = {};
  ...
  const nanoSeconds = process
    .hrtime(beginPipeline)
